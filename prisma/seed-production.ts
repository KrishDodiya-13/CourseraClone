...
// ---------------------------------------------------------------------------
//
// This file is a copy of prisma/seed.ts adapted to be invoked, once, in
// production from a secret-protected admin API route (see
// src/app/app/api/admin/seed-production/route.ts) instead of the CLI. It is
// never run automatically and is guarded by an explicit `confirm` flag passed
// by the caller in addition to that route's own secret check.
...
export async function runProductionSeed(confirm: boolean) {
  if (confirm !== true) {
    throw new Error("runProductionSeed requires explicit confirm:true from its caller.");
  }
  try {
    await main();
  } finally {
